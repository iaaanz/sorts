...
function newSort() {
  ...
  movingItem = -1;
  // tempOn = false;
  for (var i = 1; i <= valMain; i++) item[i] = i;
  ...
  item[0] = -1;
  for (var i = valMain + 1; i < valMain * 2 + 1; i++) item[i] = -1;
  $('#compCt').html('0');
  $('#moveCt').html('0');
  compCt = 0;
  copyCt = 0;
  ...
  switch (method) {
    ...
    case '3': // insertions sort
      if (j == 0) {
        ...
        multiBoxLoc.x = -1;
        multiBoxLoc.y - 1;
        for (var x = 1; x <= valMain; x++) item[x] += resizeCol;
        done = true;
        // tempOn = false;
      } else if (i == 0) {
        copyItem(1, 0);
      ...
      break;
    case '4': // merge sort
      if (lo == 1 && sortLength == valMain / 2) {
        // sortLength é o valMain / 2
